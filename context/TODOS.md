...
## Not started
- [x] **Checked the epistemic term against the real Stage 3 sweep data — gate fired.** Nothing on the tau2 sweep path had ever written a decision log before this (fixed: opt-in `EFE_DECISION_LOG_PATH` logging in `efe_agent.py`), so a dedicated EFE-only re-run across all 3 domains produced `results/stage3_tau2/efe_decision_log.jsonl` (278 real decisions). Result: **0/278 decisions driven by the epistemic term** (epistemic share 8.4%) — the same finding as the earlier 8-decision mock-agent pilot, now confirmed at full scale on genuinely varied real tasks. EFE escalated on literally every decision (100% rate in all 3 domains), so pragmatic value alone always would have picked the same policy. Full writeup: `ROADMAP.md`'s Milestone 3 (now marked GATE FIRED), `results/stage3_tau2/interpretability_report.md`.
